...
export function downloadAsPDF(note: Note) {
  ...
  doc.text("NotesGPT", pageWidth / 2, 35, { align: "center" });
  ...
  tocEntries.forEach((entry, index) => {
    ...
    const dots = ".".repeat(Math.max(0, Math.floor((contentWidth - doc.getTextWidth(entry.title) - 20) / 2)));
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text("Made by Devansh | NotesGPT - AI YouTube Notes", margin, pageHeight - 8);
...
export function downloadAsMarkdown(note: Note) {
  ...
  markdown += `> **Generated by:** NotesGPT\n\n`;
  markdown += `---\n\n`;
  ...
  markdown += `---\n\n`;
  markdown += `*Made by Devansh | NotesGPT - AI YouTube Notes*\n`;
...
export function downloadAsText(note: Note) {
  ...
  let text = `\n${divider}\n`;
  text += `  NOTESGPT - AI YOUTUBE NOTES\n`;
  text += `${divider}\n\n`;
  ...
  text += `${divider}\n`;
  text += `Made by Devansh | NotesGPT - AI YouTube Notes\n`;
  text += `${divider}\n`;
