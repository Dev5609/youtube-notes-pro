...
export function downloadAsPDF(note: Note) {
  ...
  doc.text("SummarIQ", pageWidth / 2, 35, { align: "center" });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text("Made by Devansh | SummarIQ - AI YouTube Notes", margin, pageHeight - 8);
...
export function downloadAsMarkdown(note: Note) {
  ...
  markdown += `> **Generated by:** SummarIQ\n\n`;
  markdown += `---\n\n`;
  ...
  markdown += `---\n\n`;
  markdown += `*Made by Devansh | SummarIQ - AI YouTube Notes*\n`;
...
export function downloadAsText(note: Note) {
  ...
  let text = `\n${divider}\n`;
  text += `  SummarIQ - AI YOUTUBE NOTES\n`;
  text += `${divider}\n\n`;
  ...
  text += `${divider}\n`;
  text += `Made by Devansh | SummarIQ - AI YouTube Notes\n`;
  text += `${divider}\n`;
